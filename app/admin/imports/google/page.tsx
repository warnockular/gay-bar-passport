...
export default async function AdminGoogleImportPage({ searchParams }: GoogleImportPageProps) {
  ...
  const emptySearchResult: GooglePlacesSearchResult = { candidates: [] };
  const [searchResult, venues] = await Promise.all([
    shouldSearch ? searchGooglePlacesCandidates({ city, country, neighborhood, region, searchTerm }) : Promise.resolve(emptySearchResult),
    listAdminVenues()
  ]);
  ...
  return (
    <div>
      <Badge>Google Places</Badge>
      <h1 className="mt-5 font-serif text-5xl font-semibold">Preview Google venue candidates.</h1>
      <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
        Search Google Places from the server, preview results, then stage selected candidates for admin review. Nothing is published.
      </p>
      ...
      {params.error ? (
        <p className="mt-4 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm font-semibold text-destructive" role="alert">
          Google import failed: {params.error === "no-selection" ? "Select at least one result to import." : params.error}
        </p>
      ) : null}
      {searchResult.error ? (
        <p className="mt-4 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm font-semibold text-destructive" role="alert">
      ...
      <Card className="mt-8 bg-card/90 p-6">
        <h2 className="font-serif text-3xl font-semibold">Search Google Places</h2>
      ...
      {shouldSearch ? (
        <form action={importSelectedGooglePlacesToStaging} className="mt-8 space-y-4">
          <input type="hidden" name="sourceName" value={`Google Places ${searchTerm} · ${[neighborhood, city, region].filter(Boolean).join(", ")}`} />
          ...
          {searchResult.candidates.length ? (
              ...
              <Card key={candidate.sourceId ?? candidate.name} className="bg-card/90 p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <label className="flex min-w-0 flex-1 items-start gap-3">
                    <input className="mt-1 h-4 w-4 shrink-0" type="checkbox" name="selectedGooglePlace" value={rawSelectionValue(candidate)} />
                    <span className="min-w-0">
                      <span className="block font-serif text-2xl font-semibold">{candidate.name ?? "Unnamed Google result"}</span>
            ...
            <Card className="bg-card/90 p-6 text-sm text-muted-foreground">No Google Places results returned for this search.</Card>
          )}
        </form>
      ) : null}
